...
func TestGenesisBad(t *testing.T) {
	...
	testCases := [][]byte{
		...
		[]byte(
			`{"validators":[` +
				`{"pub_key":{` +
				`"type":"celestiacore/PubKeyEd25519","value":"AT/+aaL1eB0477Mud9JMm8Sh8BIvOYlPGC9KkIUmFaE="` +
				`},"power":"10","name":""}` +
				`]}`,
		),
		// too big chain_id
...
func TestGenesisGood(t *testing.T) {
	...
	genDocBytes := []byte(
		`{
			...
			"validators": [{
				"pub_key":{"type":"celestiacore/PubKeyEd25519","value":"AT/+aaL1eB0477Mud9JMm8Sh8BIvOYlPGC9KkIUmFaE="},
				"power":"10",
				"name":""
			}],
			"app_hash":"",
